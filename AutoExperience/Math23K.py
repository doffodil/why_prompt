...
logger.add("./all_experience_record.log", encoding='utf-8')


def get_data(data_type):
    with open(f"../datasets/math_23K/data_{data_type}.json", 'r',
              encoding='utf-8') as f:
        data = json.load(f)
        f.close()
    return data
...
class AutoExpMath23K():
    ...
    def get_experience_result(self, data_type):
        ...
        for item in data:
            ...
            result['model_input'] = self.format_model_input(item,
                                                            self.parameters[
                                                                'prompt'])
            result['model_output'] = self.model.generate_output(
                result['model_input'])
